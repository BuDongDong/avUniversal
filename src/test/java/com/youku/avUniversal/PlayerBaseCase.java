...
public class PlayerBaseCase extends ItamiBaseCase {
        ...
        if (otherArgStr != null && !otherArgStr.isEmpty()) {
            try {
                ...
                for (Object each : paramArray) {
                    ...
                    } else if ("streamType".equals(((JSONObject)each).getString("name"))) {
                        streamType = ((JSONObject)each).getString("value");
                        logger.warn("设置streamType: " + streamType);
                    }
                }
            } catch (Exception e) {
    ...
    public boolean openYoukuIphoneTestVideo() {
        ...
        if (streamType != null && !streamType.isEmpty()) {
            JSONObject upsExtras = new JSONObject();
            upsExtras.put("stream_type", streamType);
            try {
                scheme = String.format("youku://play?vid=%s&point=%s&mode=1&upsExtras=%s", vid, startPoint, URLEncoder.encode(
                    JSON.toJSONString(upsExtras), "utf-8"));
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
                scheme = String.format("youku://play?vid=%s&point=%s&mode=1&quality=600", vid, startPoint);
            }
        } else {
            scheme = String.format("youku://play?vid=%s&point=%s&mode=1&quality=600", vid, startPoint);
        }
    ...
    public boolean openYoukuAndroidTestVideo() {
        ...
        String scheme;
        if (streamType != null && !streamType.isEmpty()) {
            scheme = String.format(
                "youku://play?source=stardetail\\&vid=%s\\&mode=full_horizontal\\&point=%s\\&stream_type=%s", vid,
                startPoint, streamType);
        } else {
            scheme = String.format(
                "youku://play?source=stardetail\\&vid=%s\\&mode=full_horizontal\\&quality=400\\&point=%s", vid,
                startPoint);
        }
        Router.AnroidSchemeLaunch(driver, scheme, DEVICE.getDeviceId());
        return true;
    }
